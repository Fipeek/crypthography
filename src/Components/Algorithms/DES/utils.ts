export const initialPermutation: number[] = [
...
export const keyPermutation: number[] = [
...
export const convertToBinaryArray = (input: string) =>
  input.split("").map((char) => convertToBinary(char));
...
export const applyPermutation = (
  ...
  const output = new Array(permutationTable.length).map(
    (el, index) => (el = input[permutationTable[index] - 1])
  );
